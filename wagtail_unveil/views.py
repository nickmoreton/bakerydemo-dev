...
from .helpers.generic_helpers import get_generic_urls
...
class UnveilGenericReportView(ReportView):
    index_url_name = "unveil_generic_report"
    index_results_url_name = "unveil_generic_report_results"
    header_icon = "cogs"
    template_name = "wagtail_unveil/unveil_url_report.html"
    results_template_name = "wagtail_unveil/unveil_url_report_results.html"
    ...
    def get_queryset(self):
        ...
        # Get URLs from different sources using helper functions
        # Get max_instances from settings with a default of 5 (generic models are typically few)
        max_instances = getattr(settings, 'WAGTAIL_UNVEIL_MAX_INSTANCES', 1)
        base_url = "http://localhost:8000"  # Default base URL
        
        # Collect generic model URLs
        generic_urls = get_generic_urls(output, base_url, max_instances, self.request.user)
